...
PHPExcel_Settings::setCacheStorageMethod($cacheType);
...
for ($i = 0; $i < 10; $i++) {
    ...
    $styles[] = $style;
	$objPHPExcel->addCellXf($style);
}

$notifyCache = method_exists('PHPExcel_Worksheet', 'setNotifyCacheControllerEnabled');

if ($notifyCache) {
	$worksheet->setNotifyCacheControllerEnabled(false);
}
